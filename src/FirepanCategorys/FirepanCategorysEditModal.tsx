/**
 * 火入個別情報登録画面（FirepanCategorys EditWindow.xaml 相当）
 */
...
import {
  computeFirepanMachineLabel,
  computeOverallJudge,
  formToFirepanCategoryUpsertPayload
} from "./buildFirepanCategoryEditForm";
import type { FirepanCategoryEditFormData } from "./firepanCategoryEditTypes";
import { upsertFirepanCategoryAtom } from "./store";
import "../Factory2LotManufacture/factory2LotEditModal.css";
import "../BlendCategorys/blendCategoryEditModal.css";
import "./firepanCategoryEditModal.css";
...
function EditInput({
  ...
  return (
    <input
      className={`fpEditInput fpEditAlign${align === "left" ? "Left" : align === "center" ? "Center" : "Right"}`}
      style={{ width }}
      type="text"
      value={value}
...
export function FirepanCategorysEditModal({ form: initialForm, onClose }: Props) {
  const upsert = useSetAtom(upsertFirepanCategoryAtom);
  ...
  const setField = useCallback(
    <K extends keyof FirepanCategoryEditFormData>(key: K, value: FirepanCategoryEditFormData[K]) => {
      setForm((p) => ({ ...p, [key]: value }));
    },
    []
  );

  const overallJudgeBefore = useMemo(
    () =>
      computeOverallJudge(
        form.sensualTestColorBefore,
        form.sensualTestTasteBefore,
        form.sensualTestAromaBefore
      ),
    [form.sensualTestColorBefore, form.sensualTestTasteBefore, form.sensualTestAromaBefore]
  );

  const overallJudgeAfter = useMemo(
    () =>
      computeOverallJudge(
        form.sensualTestColorAfter,
        form.sensualTestTasteAfter,
        form.sensualTestAromaAfter
      ),
    [form.sensualTestColorAfter, form.sensualTestTasteAfter, form.sensualTestAromaAfter]
  );

  const firepanMachineLabel = useMemo(
    () =>
      computeFirepanMachineLabel(form.useDeviceUnit1, form.useDeviceUnit2, form.useDeviceUnit3),
    [form.useDeviceUnit1, form.useDeviceUnit2, form.useDeviceUnit3]
  );
  ...
  const handleRegist = useCallback(() => {
    ...
    void runBusy(async () => {
      const success = await upsert(formToFirepanCategoryUpsertPayload(form));
      if (success) {
        onClose();
      }
    }, "登録中…");
  ...
  return (
    <EditModalOverlay mode="view" onClose={onClose} className="fpEditOverlay">
      <div
        className="fpEditPanel bcEditPanel modalPanel"
        role="dialog"
        aria-modal="true"
        aria-labelledby="fpEditTitle"
        onClick={(e) => e.stopPropagation()}
      >
        <h2 className="bcEditWindowTitle">火入個別情報登録画面</h2>
        <div className="bcEditToolbar f2EditToolbar">
          <span id="fpEditTitle" className="bcEditHeaderTitle">
            製造報告書（火入）
          </span>
        ...
        <div className="f2EditBlock fpEditMachineBlock">
          <div className="fpEditMachineBadge" aria-hidden>
            機器
            <br />
            設定
          </div>
          <div className="fpEditMachineGrid" role="group" aria-label="機器設定">
            <div className="fpEditMachineHead">排気</div>
            <div className="fpEditMachineHead">本機</div>
            <div className="fpEditMachineHead">設定温度</div>
            <div className="fpEditMachineHead">バーナーON</div>
            <div className="fpEditMachineHead">遠赤</div>
            <div className="fpEditMachineHead">投入量</div>
            <div className="fpEditMachineHead">温度</div>
            <div className="fpEditMachineHead">火入機</div>

            <div className="fpEditMachineSubHead">左</div>
            <div className="fpEditMachineSubHead">右</div>
            <div className="fpEditMachineSubHead">左</div>
            <div className="fpEditMachineSubHead">中</div>
            <div className="fpEditMachineSubHead">右</div>

            <div className="fpEditMachineValue">
              <EditInput
                value={form.firValue1}
                onChange={(v) => setField("firValue1", v)}
                width={40}
                align="center"
              />
            </div>
            <div className="fpEditMachineValue">
              <EditInput
                value={form.firValue2}
                onChange={(v) => setField("firValue2", v)}
                width={40}
                align="center"
              />
            </div>
            <div className="fpEditMachineValue">
              <EditInput
                value={form.firValue3a}
                onChange={(v) => setField("firValue3a", v)}
                width={40}
                align="center"
              />
            </div>
            <div className="fpEditMachineValue">
              <EditInput
                value={form.firValue3b}
                onChange={(v) => setField("firValue3b", v)}
                width={40}
                align="center"
              />
            </div>
            <div className="fpEditMachineValue">
              <EditInput
                value={form.firValue4a}
                onChange={(v) => setField("firValue4a", v)}
                width={40}
                align="center"
              />
            </div>
            <div className="fpEditMachineValue">
              <EditInput
                value={form.firValue4b}
                onChange={(v) => setField("firValue4b", v)}
                width={40}
                align="center"
              />
            </div>
            <div className="fpEditMachineValue">
              <EditInput
                value={form.firValue4c}
                onChange={(v) => setField("firValue4c", v)}
                width={40}
                align="center"
              />
            </div>
            <div className="fpEditMachineValue">
              <EditInput
                value={form.firValue5}
                onChange={(v) => setField("firValue5", v)}
                width={40}
                align="center"
              />
            </div>
            <div className="fpEditMachineValue">
              <EditInput
                value={form.firValue6}
                onChange={(v) => setField("firValue6", v)}
                width={40}
                align="center"
              />
            </div>
            <div className="fpEditMachineValue">
              <EditInput
                value={form.firValue7}
                onChange={(v) => setField("firValue7", v)}
                width={40}
                align="center"
              />
            </div>
            <div className="fpEditMachineValue fpEditMachineValueReadonly">
              <ReadonlyText value={firepanMachineLabel} />
            </div>
          </div>
        </div>

        <div className="f2EditBlock fpEditSensualBlock">
          <div className="fpEditSectionBadge">官能検査</div>
          <div className="fpEditSensualGrid">
            <div className="fpEditSensualHeadRow">
              <LabelCell width={80} className="fpEditMiniHead" />
              <LabelCell width={100} className="fpEditMiniHead">
                水色
              </LabelCell>
              <LabelCell width={100} className="fpEditMiniHead">
                味
              ...
              <LabelCell width={140} className="fpEditMiniHead">
                火入時留意点・所見
              </LabelCell>
            </div>
            <div className="fpEditSensualDataRow">
              <ValueCell width={80}>
                <ReadonlyText value="火入前○" />
              </ValueCell>
              <ValueCell width={100} className="bcEditSensualInputCell">
                <EditInput
                  value={form.sensualTestColorBefore}
                  onChange={(v) => setField("sensualTestColorBefore", v)}
                  width={80}
                  align="center"
                />
              </ValueCell>
              <ValueCell width={100} className="bcEditSensualInputCell">
                <EditInput
                  value={form.sensualTestTasteBefore}
                  onChange={(v) => setField("sensualTestTasteBefore", v)}
                  width={80}
                  align="center"
                />
              </ValueCell>
              <ValueCell width={100} className="bcEditSensualInputCell">
                <EditInput
                  value={form.sensualTestAromaBefore}
                  onChange={(v) => setField("sensualTestAromaBefore", v)}
                  width={80}
                  align="center"
                />
              </ValueCell>
              <ValueCell width={100} align="center">
                <ReadonlyText value={overallJudgeBefore} />
              </ValueCell>
              <ValueCell width={140}>
                <EditInput
                  value={form.sensualTestCommentBefore}
                  onChange={(v) => setField("sensualTestCommentBefore", v)}
                  width={136}
                  align="left"
                />
              </ValueCell>
            ...
            <div className="fpEditSensualDataRow">
              <ValueCell width={80}>
                <ReadonlyText value="火入後●" />
              </ValueCell>
              <ValueCell width={100} className="bcEditSensualInputCell">
                <EditInput
                  value={form.sensualTestColorAfter}
                  onChange={(v) => setField("sensualTestColorAfter", v)}
                  width={80}
                  align="center"
                />
              </ValueCell>
              <ValueCell width={100} className="bcEditSensualInputCell">
                <EditInput
                  value={form.sensualTestTasteAfter}
                  onChange={(v) => setField("sensualTestTasteAfter", v)}
                  width={80}
                  align="center"
                />
              </ValueCell>
              <ValueCell width={100} className="bcEditSensualInputCell">
                <EditInput
                  value={form.sensualTestAromaAfter}
                  onChange={(v) => setField("sensualTestAromaAfter", v)}
                  width={80}
                  align="center"
                />
              </ValueCell>
              <ValueCell width={100} align="center">
                <ReadonlyText value={overallJudgeAfter} />
              </ValueCell>
              <ValueCell width={140}>
                <EditInput
                  value={form.sensualTestCommentAfter}
                  onChange={(v) => setField("sensualTestCommentAfter", v)}
                  width={136}
                  align="left"
                />
              </ValueCell>
