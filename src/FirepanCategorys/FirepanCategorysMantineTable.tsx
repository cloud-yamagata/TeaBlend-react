...
const COLUMNS: MantineScrollTableColumn<FirepanCategoryRow>[] = [
  {
    key: "workDate",
    label: "製造日",
...
type Props = {
  ...
  onRowSelect: (row: FirepanCategoryRow) => void;
  searchExecuted?: boolean;
};

export const FirepanCategorysMantineTable = memo(function FirepanCategorysMantineTable({
  rows,
  selectedRowId,
  onRowSelect,
  searchExecuted = true
  ...
  return (
    <MantineScrollTable
      ...
      getRowId={(row) => row.id}
      columns={columns}
      pagination={listTablePagination.firepanCategorys}
      minTableWidth={1050}
      showFilter={false}
      emptyMessage={emptyMessage}
      selectedRowId={selectedRowId}
      onRowSelect={onRowSelect}
      className="firepanCategoryMantineRoot"
      scrollClassName="firepanCategoryMantineScroll mantineScrollTableScroll"
      tableClassName="firepanCategoryMantineTable mantineScrollTable"
    />
  );
});
