...
const COLUMNS: MantineScrollTableColumn<FinishCategoryRow>[] = [
  {
    key: "workDate",
    label: "製造日",
...
type Props = {
  ...
  onRowSelect: (row: FinishCategoryRow) => void;
  searchExecuted?: boolean;
};

export const FinishCategorysMantineTable = memo(function FinishCategorysMantineTable({
  rows,
  selectedRowId,
  onRowSelect,
  searchExecuted = true
  ...
  return (
    <MantineScrollTable
      ...
      getRowId={(row) => row.id}
      columns={columns}
      pagination={listTablePagination.finishCategorys}
      minTableWidth={1050}
      showFilter={false}
      emptyMessage={emptyMessage}
      selectedRowId={selectedRowId}
      onRowSelect={onRowSelect}
      className="finishCategoryMantineRoot"
      scrollClassName="finishCategoryMantineScroll mantineScrollTableScroll"
      tableClassName="finishCategoryMantineTable mantineScrollTable"
    />
  );
});
